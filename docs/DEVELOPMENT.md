...
An unknown key at any level and a missing `label` are checked once, before any file is profiled, naming the JSON path to the offence. A label the taxonomy does not carry and a label whose Frictionless type the Data Package v2 profile does not admit (`container.array.comma_separated` is the one of 251 that maps to `list`) are checked in that same pass, naming the column, the stem and the label. A declared stem no input matches is checked there too, naming the stem and the file. A declared column the file does not have is checked per file, after that file is read: in a `--files` batch, a file declared later in the nominations file can fail this check after earlier files in the run have already been profiled and written to `--out-dir`. The reverse is not an error — an input with no entry in the file is profiled by inference, which is the ordinary `--files` batch case. The asymmetry with inference is deliberate: an *inferred* `list` still emits, because that is a mid-run model answer about data the caller cannot change, while a *nominated* `list` is a deliberate declaration made before any work starts and can be refused while the person is still holding the file they can fix.
