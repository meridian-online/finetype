...
- `.orbit/choices/0031-table-level-schema-via-finetype-schema.md` — superseded record (status update)
...
- **A** is the cleanest and matches the visibility-cleanup spec's stated plan. Cost: one new file + one frontmatter flip on 0031. The MADR template (`.orbit/choices/_template.md`) is short. The MADR can be written from the memos almost verbatim (`schema-profile-overlap` lines 154-204 already supplies Considered Options + Decision Outcome + Consequences).
