...
- Use `orbit` verbs for spec tracking — never markdown TODO lists.
...
- Use `orbit task open / claim / update / done` for cross-session task tracking — anything that must outlast the session, sync via git, or attach to a spec. `TaskCreate` is fine for in-session working structure; mirror to `orbit task` only when the task needs to persist.
