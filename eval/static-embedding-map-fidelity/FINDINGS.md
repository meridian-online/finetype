...
**Answered 2026-08-28 by `map_fidelity.py`, in one pass over three corpora at seed 42. Every figure below is read from `results.json`, committed beside it. **Three tables are *generated* from that file** — the measurement, the gap attribution and the both-stages overlap — and `check_results.py` compares all three against it, so a figure inside one of those blocks cannot outlive the run that produced it. **The prose around them is not generated and is not checked.** Roughly forty derived figures sit in the sentences between the tables; they were correct when written and nothing reddens if a re-run moves them. Read a prose figure as a reading of the tables, and check it against them if it matters.

*This sentence used to claim the guarantee covered the whole document. It covered one table. Three corrections each retyped a derived figure and one was wrong every time, the third inside the retraction's own evidence; a fourth review found the claim itself false by planting a wrong number outside the block and watching the check pass. The guarantee is now stated at the size it actually is.* Re-run rather than quoting from memory: vectors from two model versions are not comparable, so this question returns whenever a model is bumped.**
...
**But the kNN-overlap column, which is the one the deferral cited, barely moves at all.** Across the whole ladder it goes 0.1318 → 0.1603 on bodies, 0.2776 → 0.2861 on subjects, and 0.4030 → 0.3998 on column names — the last one *down*. The same models gained 0.073, 0.086 and 0.037 of ranked lift over the same span, against 0.028, 0.009 and −0.003 of neighbourhood agreement.
