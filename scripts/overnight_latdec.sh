#!/usr/bin/env bash
# scripts/overnight_latdec.sh — latitude->decimal candidate full retrain (ac-04).
#
# Spec 2026-06-06-latitude-decimal-hard-negative-retrain. The proxy pre-check
# (ac-03) returned GO, authorising this overnight run. Trains the candidate at
# full depth — 3 ReLU+BN seeds × 50 epochs — on the ALREADY-BUILT candidate FTMB
# (output/multibranch-training/latdec-blend.ftmb), the inverse of v24's bet:
# feature floats v19 wrongly grabs as latitude, pushed back to decimal_number.
#
...
# architecture comparison. No GELU arm.
#
# The FTMB is built and validated by build_latdec_distilled.py + the cap
# override in prepare_multibranch_data.py; this script reuses it (no data prep).
# It does NOT cherry-pick a seed or swap models/default — the gold-anchor
# re-score (ac-04, scripts/score_gold_anchor.py) across all 3 seeds is the
# judge, and promotion (ac-06) is a separate reviewed decision.
#
# Usage:
#   scripts/overnight_latdec.sh                 # full 3-seed run
#   scripts/overnight_latdec.sh --epochs N       # override epoch cap
#   scripts/overnight_latdec.sh --dry-run        # show config, don't train
set -eo pipefail
...
LOG_FILE="$LOG_DIR/overnight-latdec.log"
...
FTMB_FILE="output/multibranch-training/latdec-blend.ftmb"

# name|config|seed|lr|wd  — ReLU+BN, 3 seeds, identical hyperparameters to v19.
RUNS=(
    "sherlock-latdec-relu-s42|$RELU_CONFIG|42|0.0001|0.0001"
    "sherlock-latdec-relu-s43|$RELU_CONFIG|43|0.0001|0.0001"
    "sherlock-latdec-relu-s44|$RELU_CONFIG|44|0.0001|0.0001"
)

echo "================================================================"
echo " latitude->decimal candidate retrain (ac-04)"
...
print('  PASS')
dec_n = label_counts.get('representation.numeric.decimal_number', 0)
print(f'Gate 5 (decimal signal): {dec_n} rows (gate >=600)')
if dec_n < 600: print('FAIL: hard-neg signal collapsed under cap'); sys.exit(1)
print('  PASS')
lat_n = label_counts.get('geography.coordinate.latitude', 0)
print(f'Gate 6 (latitude preserved): {lat_n} rows (advisory — coordinate-recall guard)')
print('')
...
echo "================================================================"
echo " latitude->decimal retrain — Summary"
...
echo ""
echo "Next (ac-04): gold-anchor re-score across all 3 seeds —"
echo "  scripts/score_gold_anchor.py predict/score per seed vs the v19 anchor"
echo "  (output/gold-eval-anchor/metrics_v19-models-default_2026-06-05.tsv)."
echo "  Success: decimal recall >0.600 AND latitude precision >0.714, latitude"
echo "  recall holds 1.000, no per-family regression. models/default NOT swapped."
echo "Next (ac-05): full-label-space post-train Sense-distribution drift vs v19."
echo ""
echo "Log: $LOG_FILE"
echo "================================================================"
