...
# (output/multibranch-training/mfg-blend-coords.ftmb), v19 ReLU recipe verbatim:
...
FTMB_FILE="output/multibranch-training/mfg-blend-coords.ftmb"
RELU_CONFIG="models/sherlock-v13-config.json"
EPOCHS=100
PATIENCE=15
BATCH_SIZE=32
# Single seed for the structural hypothesis test (hierarchical head ~1.8x slower/epoch
# than flat). One seed gives a directional corpus-honest verdict; best-of-3 confirms
# later only if this clears or dramatically improves the numeric collateral.
SEEDS=(42)
BIN="$PROJECT_DIR/target/release/finetype"
LOG="output/mining-factory/coord-only/train_hier.log"

exec > >(tee -a "$LOG") 2>&1

echo "================================================================"
echo " coord-only candidate train — 3 seeds, HIERARCHICAL head (choice 0097 / research arm)"
echo " FTMB:   $FTMB_FILE"
...
for seed in "${SEEDS[@]}"; do
    name="sherlock-mfg-coords-hier-s${seed}"
...
for seed in "${SEEDS[@]}"; do
    echo "  models/sherlock-mfg-coords-hier-s${seed}"
done
echo "================================================================"
