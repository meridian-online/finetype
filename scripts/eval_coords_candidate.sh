...
for s in "${SEEDS[@]}"; do
    R="models/sherlock-mfg-coords-relu-s${s}/results.json"
...
MODEL="models/sherlock-mfg-coords-relu-s${BEST_SEED}"
echo ""
...
GOLD_PRED="$LFDIR/predictions_mfg-coords.tsv"
...
"$VPY" scripts/score_gold_anchor.py score \
    ...
    --model-name "mfg-coords-s${BEST_SEED}" --out-dir "$LFDIR" || echo "  score failed"
...
python3 scripts/drift_report.py \
    ...
    "$LFDIR/sense_dist_mfg-coords-full.json" \
...
FINETYPE_MODEL="$MODEL" python3 scripts/gittables_corpus_pass.py \
    ...
    --execute --jobs 8 --out-dir "$LFDIR/sample_pass" || echo "  corpus pass failed"

# ── 4. Corpus-honest gate (BLOCKING) ────────────────────────────────────
echo ""; echo "── [4/4] Corpus-honest gate (H05, BLOCKING) ──"
CAND_PARQUET="$LFDIR/sample_pass/corpus_pass/columns.parquet"
python3 scripts/corpus_honest_gate.py \
    --candidate "$CAND_PARQUET" \
    --label "mfg-coords-s${BEST_SEED}" \
...
echo " Gold report:  $LFDIR/report_mfg-coords-s${BEST_SEED}_*.md"
