#!/usr/bin/env bash
# scripts/eval_amount_veto.sh — gold + corpus-honest gate for the coordinate
...
set -uo pipefail
cd "$(cd "$(dirname "$0")/.." && pwd)"
OUT="output/amount-veto"; mkdir -p "$OUT"
...
FINETYPE_MODEL="$MODEL" "$VPY" scripts/score_gold_anchor.py predict \
  ...
  --out "$OUT/predictions_amount_veto.tsv" || echo "  predict failed"
"$VPY" scripts/score_gold_anchor.py score \
  --gold "$GOLD" --predictions "$OUT/predictions_amount_veto.tsv" \
  --model-name amount-veto --out-dir "$OUT" || echo "  score failed"
...
python3 scripts/corpus_honest_gate.py \
  ...
  --label amount-veto | tee "$OUT/corpus_honest_gate.txt"
