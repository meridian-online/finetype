...
cd "$(cd "$(dirname "$0")/.." && pwd)"
LFDIR="output/mining-factory/locale-format"
...
echo "================================================================"
echo " ac-06 honest evaluation — locale-format candidate   $(date)"
echo "================================================================"

# ── Pick best seed by val_accuracy ──────────────────────────────────────
BEST_SEED=""; BEST_ACC=0
for s in "${SEEDS[@]}"; do
    R="models/sherlock-mfg-localefmt-relu-s${s}/results.json"
...
MODEL="models/sherlock-mfg-localefmt-relu-s${BEST_SEED}"
echo ""
...
python3 scripts/drift_report.py \
    ...
    "$LFDIR/sense_dist_mfg-localefmt-full.json" \
...
python3 scripts/corpus_honest_gate.py \
    ...
    --label "mfg-localefmt-s${BEST_SEED}" \
...
echo " Gold report:  $LFDIR/report_mfg-localefmt-s${BEST_SEED}_*.md"
