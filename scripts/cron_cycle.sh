...
if (( DRY_RUN )); then
    ...
    if ! command -v "$CLAUDE_BIN" >/dev/null 2>&1; then
        ...
        PROMPT="You are the FineType cron-firing agent. Read scripts/contracts/2026-05-03-gittables-90-percent-roundtrip.yaml and run one cycle per §2 standing orders. Cycle ID: $CYCLE_ID. Halt and surface (do not improvise) on any unmatched state."
