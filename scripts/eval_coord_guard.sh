#!/usr/bin/env bash
# scripts/eval_coord_guard.sh — gold + corpus-honest gate for the coordinate
...
set -uo pipefail
cd "$(cd "$(dirname "$0")/.." && pwd)"
OUT="output/coord-promote-guard"; mkdir -p "$OUT"
...
FINETYPE_MODEL="$MODEL" "$VPY" scripts/score_gold_anchor.py predict \
  ...
  --out "$OUT/predictions_coord_guard.tsv" || echo "  predict failed"
"$VPY" scripts/score_gold_anchor.py score \
  --gold "$GOLD" --predictions "$OUT/predictions_coord_guard.tsv" \
  --model-name coord-guard --out-dir "$OUT" || echo "  score failed"
...
python3 scripts/corpus_honest_gate.py \
  ...
  --label coord-guard | tee "$OUT/corpus_honest_gate.txt"
