...
- [x] All diagnostics land under diagnostics/ as reproducible TSVs with a committed regeneration script. (diagnostics/: corpus_counts.tsv, jaccard_matrix.tsv, confusion.tsv, predictions.tsv, predictions_post.tsv, confidence_topk.tsv, target_delta.tsv, full_eval_delta.tsv, profile_results_pre.csv, profile_results_post.csv, v16_corpus_hash.txt; scripts/amvg/ac0N_*.py.)
...
- [x] ac-08: Full-eval delta on 448-row manifest (v16 pre-fix vs v16+ac-06 post-fix). Target-delta.tsv + full_eval_delta.tsv written. Per detour: v19 smoke retrain is informationally null because the mechanism is pipeline-only — ac-06 does not touch training corpus or model architecture. Artefacts: `diagnostics/profile_results_pre.csv`, `diagnostics/profile_results_post.csv`, `diagnostics/target_delta.tsv`, `diagnostics/full_eval_delta.tsv`.
- [x] ac-09 (gate): **GO**. target_fixes=11, target_regressions=0, net_lift=**11** (threshold >=3). All 11 target variants now correctly classified. `net_lift` is outside the {3,4} tight band — constraint #13's 3-seed confirmation caveat does not apply. Verdict: `diagnostics/v19_smoke_verdict.txt`.
- [x] ac-10 (gate): **PASS**. non_target_fixes=18, non_target_regressions=8, regression_delta=**+10** (threshold >=-1). Fix is strictly-positive on non-target columns as well. Verdict: `diagnostics/v19_smoke_regression_verdict.txt`.
- [x] ac-11: MADR authored — `orbit/decisions/0066-v19-retrain-hard-gate.md`.
- [x] ac-12: MADR authored — `orbit/decisions/0067-framing-correction-retrain-is-not-the-lever.md`.
