...
**Parent spec:** `orbit/specs/2026-04-18-v16-data-audit-retrain/` (m-18 trained the model)
...
- **N=1 email regression** (`orbit/specs/2026-04-20-v16-n1-email-regression/card.md`) — narrow but real. v16 fails on a single email value in column mode where v14 succeeded. Workaround in smoke tests; investigation deferred.
- **CI hygiene — decouple download-model.sh from `models/default`** (`orbit/specs/2026-04-20-ci-decouple-default-symlink/spec.yaml`) — the root cause of this release's "publish HF first, then flip symlink" dance. Next promotion PR should not need this ordering. **Addressed:** option A (env-var pin) specced and implemented — `FINETYPE_CI_MODEL` in workflow-level `env:` blocks. See also the 7-finding spec review (`review-spec-2026-04-20.md`) and progress tracker (`progress.md`).
