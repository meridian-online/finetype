...
Engine: `finetype profile` (isolated single-column, full Sense+Sharpen). Header: leaf-name. Rows/column: 20. Binary: `target/release/finetype` (finetype 0.6.52).

**Round-trip: 242/251 (96.4%).**
...
- New unacknowledged fails: **1**
- Graduated (acknowledged gap now passes): **0**

### New unacknowledged fails
- `technology.filesystem.filename` → `technology.internet.hostname`
...
| `representation.discrete.ordinal` | `representation.text.word` |
