...
| 4 | version_string | technology.development.version | ~100–130 distinct | **SHIPPED 2026-07-14** | `version_string_recovery` guard: LOAD-BEARING header gate (`version_corroborates`, date tokens veto) + `is_version_string` (SemVer shape + 4-digit-year veto for dates/calver). FIRE_ON = residual + integer/decimal_number (the veto-demoted-to-unknown numerics — pipeline-order lesson). NO distinct floor (constant version cols are normal). 414 recoveries 0 FP (348 = one replicated `ver=1.6.1` dump, thin diversity as warned). Gate GO zero bands, gold flat 0-regression, repr flat. `output/version-string/finding.md` |
