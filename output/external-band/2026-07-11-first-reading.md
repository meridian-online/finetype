...
- **The full pool (90/145) is being split** model-vs-gold the same way (`output/external-band/full_pool_split_2026-07-11.md` when complete); the seam recurs across it (majestic `TLD`→continent, compound `code`→postal, chicago `iucr`→compact_dmy), and there are more gold-error candidates (a cluster of salary/price columns gold-labelled `finance.amount` where the model says `decimal_number`).
