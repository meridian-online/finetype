...
These are **gold-fix candidates** (panel-proposes / author-ratifies, per the growth policy). They also make a point that lands for the whole review: the band **cross-checks the fixture against real full-column data**, and the fixture had errors the curated-hard process missed.
...
- **Two real actions.** (a) **The 5 gold corrections** are the cheap immediate win (author-ratify the 4 date leaves → `iso`, entity_status → `boolean.terms`). (b) **The short-code / org-name → geography seam** is the genuine model target — 8 of the trio's errors and directly the audit's 1a/1c mechanism; it is a Sharpen-guard or retrain job, gated by the blocking corpus-honest gate, not a quick edit.
- **The full pool (90/145) is not yet split** model-vs-gold the way the trio is; expect a similar mix, and the seam recurs across it (majestic `TLD`→continent, compound `code`→postal, chicago `iucr`→compact_dmy).
