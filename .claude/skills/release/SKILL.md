...
### meridian.online docs (separate surface — surface this every binary release)

`release.yml` does **not** touch `meridian-online/web`. The finetype docs there carry two
kinds of claim that go stale on a release and that nothing in this repo can see:

1. **The type registry.** `web/src/data/taxonomy.json` is generated by
   `web/scripts/build-taxonomy.py` from `finetype taxonomy` output, and every type page plus
   every "N semantic types" sentence is rendered from it. The script resolves `finetype` on
   `PATH`, so it stamps whatever binary happened to be installed.
2. **The accuracy figures.** `content/docs/projects/finetype/index.mdx` and `faq.mdx` publish
   label / domain / actionability percentages. These do **not** move with a release unless the
   eval behind them is re-run, and they are the numbers a prospective user reads.

**Ask after every binary release, the same way the community extension is surfaced:**

> meridian.online's type registry was generated from finetype **{web_stamp}** (this release is
> **{new_version}**). Refresh it now? [y/N]

Read `{web_stamp}` from the snapshot's own generator block:

```bash
jq -r '.generator.finetype_version // "unstamped"' ../web/src/data/taxonomy.json
```

**`unstamped` means the snapshot predates the stamp and its provenance is unknown — treat it
as stale.**

**Refresh only when this release changed what the registry publishes** — a taxonomy leaf added
or renamed, a `validation`/`transform`/`samples` edit, a description change. A release that
touches only Rust, CI or docs leaves `labels/` alone and the snapshot is still true. Check with:

```bash
git diff --stat <previous-tag>..HEAD -- labels/
```

If **yes** — in `web`:

1. **Upgrade the local binary first, and verify it.** `brew upgrade finetype && finetype
   --version` must print `{new_version}`. This is the step that actually goes wrong: the
   formula updates at release time but the installed binary does not, so a regeneration run
   straight after a release stamps the **old** version while looking like it worked.
2. `finetype taxonomy --full --output json > /tmp/ft_full.json`
3. `finetype taxonomy '*' -o json-schema > /tmp/ft_schemas.json`
4. `python3 scripts/build-taxonomy.py /tmp/ft_full.json /tmp/ft_schemas.json`
5. `node scripts/check-taxonomy-count.mjs` — fails when a count in `content/` disagrees with the
   regenerated registry, and names the file and line. This is the gate that turns a stale
   snapshot into a red build rather than a wrong sentence on a live page.
6. Open a PR. **Do not push to `web`'s `main`** — it auto-deploys meridian.online, and that is
   a HOLD.

**The accuracy figures are not covered by any of the above, and this is deliberate.** No
instrument re-derives them, so nothing here can tell you whether they are still true. If this
release changed model behaviour — a `models/` change, or a Sharpen rule that moves labels —
say so explicitly rather than leaving the percentages to stand by default:

> This release changed typing behaviour. The accuracy figures on meridian.online trace to a
> named earlier release and nothing re-measures them. Flag for re-measurement? [y/N]
