...
fn generate_embedded_models(models_base: &Path, labels_base: &Path) {
    ...
    // Detect model type: tiered (has tier_graph.json), flat CharCNN (has labels.json),
    // or multi-branch (has label_map.json + config.json). Multi-branch models are loaded
