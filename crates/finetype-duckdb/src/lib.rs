...
/// Resolve Model2Vec resources for the multi-branch embed + header branches.
///
/// Without these the embed/header branches run on zeroed features and column
/// classification is materially worse (dates/emails collapse to plain_text),
/// so this mirrors the CLI's `set_model2vec` wiring. Resolution order:
///   1. `FINETYPE_MODEL2VEC_DIR` — explicit override.
///   2. A `model2vec/` dir next to the resolved model dir, or inside it.
///   3. The `model2vec/` subdir of the HuggingFace repo (cached after first use).
///
/// Returns None (logged) rather than failing — the extension stays loadable in
/// degraded mode if Model2Vec is genuinely unavailable.
fn resolve_model2vec(model_dir: &std::path::Path) -> Option<Model2VecResources> {
    if let Ok(dir) = std::env::var("FINETYPE_MODEL2VEC_DIR") {
        return Model2VecResources::load(&dir)
            .map_err(|e| tracing::warn!("FINETYPE_MODEL2VEC_DIR set but unloadable: {e}"))
            .ok();
    }

    // Local layout: a `model2vec` sibling of the model dir, or nested under it.
    for candidate in [
        model_dir.join("model2vec"),
        model_dir.parent().map(|p| p.join("model2vec")).unwrap_or_default(),
    ] {
        if candidate.join("tokenizer.json").exists() {
            return Model2VecResources::load(&candidate)
                .map_err(|e| tracing::warn!("Model2Vec at {} unloadable: {e}", candidate.display()))
                .ok();
        }
    }

    // HuggingFace Hub: fetch the two files from the model2vec/ subdir.
    let fetch = || -> std::result::Result<Model2VecResources, Box<dyn Error>> {
        let repo = hf_hub::Repo::new(HF_REPO.to_string(), hf_hub::RepoType::Model);
        let api = hf_hub::api::sync::Api::new()?.repo(repo);
        let tokenizer_bytes = std::fs::read(api.get(M2V_TOKENIZER)?)?;
        let model_bytes = std::fs::read(api.get(M2V_MODEL)?)?;
        Ok(Model2VecResources::from_bytes(&tokenizer_bytes, &model_bytes)?)
    };
    fetch()
        .map_err(|e| tracing::warn!("Failed to fetch Model2Vec from HuggingFace: {e}"))
        .ok()
}
...
fn get_column_classifier() -> &'static ColumnClassifier {
    COLUMN_CLASSIFIER.get_or_init(|| {
        ...
        if let Some(m2v) = resolve_model2vec(&model_dir) {
