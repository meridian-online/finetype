...
// ── ac-04a: ColumnScanStats free-stats plumbing ─────────────────────
//
// Spec 2026-06-16-duckdb-shellout-ingestion ac-04a. The struct computes
// per-column full-column stats and exposes the affirmative mirrors of the
// shipped demotion vetoes. Parity-gated: nothing in the live classify flow
// consumes it yet, so these tests are the whole proof it is correct.
...
#[test]
fn scan_stats_basic_counts() {
    let s = ColumnScanStats::from_values(&col(&["1", "2", "2", "", "  ", "x"]));
    assert_eq!(s.non_empty, 4, "empty/whitespace dropped");
    assert_eq!(s.distinct, 3, "distinct over {{1,2,x}}");
    assert_eq!(s.int_count, 3, "1,2,2 parse as int");
    assert_eq!(s.int_distinct, 2, "{{1,2}}");
    assert_eq!(s.int_min, Some(1));
    assert_eq!(s.int_max, Some(2));
}

#[test]
fn scan_stats_cardinality_ratio() {
    let uniq = ColumnScanStats::from_values(&col(&["a", "b", "c", "d"]));
    assert!((uniq.cardinality_ratio() - 1.0).abs() < 1e-6);
    let repeat = ColumnScanStats::from_values(&col(&["a", "a", "a", "b"]));
    assert!((repeat.cardinality_ratio() - 0.5).abs() < 1e-6);
    let empty = ColumnScanStats::from_values(&col(&["", "  "]));
    assert_eq!(empty.cardinality_ratio(), 0.0, "empty column → 0, no panic");
}

#[test]
fn scan_stats_is_binary_domain() {
    assert!(ColumnScanStats::from_values(&col(&["0", "1", "1", "0"])).is_binary_domain());
    assert!(ColumnScanStats::from_values(&col(&["0", "0", "0"])).is_binary_domain());
    // A 0/1/2 count column is NOT a binary domain.
    assert!(!ColumnScanStats::from_values(&col(&["0", "1", "2"])).is_binary_domain());
    // true/false text is not the integer {0,1} domain this predicate guards.
    assert!(!ColumnScanStats::from_values(&col(&["true", "false"])).is_binary_domain());
    // A stray non-integer breaks the strict all-integer requirement.
    assert!(!ColumnScanStats::from_values(&col(&["0", "1", "x"])).is_binary_domain());
}

#[test]
fn scan_stats_increment_parity_with_veto() {
    // is_increment() is the affirmative mirror of values_form_increment ==
    // Some(true). Assert they agree across a spread of shapes so a future
    // refactor of either cannot silently diverge.
    let cases: Vec<Vec<String>> = vec![
        col(&["0", "1", "2", "3", "4", "5"]),           // contiguous run
        col(&["100", "101", "102", "103", "104"]),      // contiguous, offset
        col(&["0", "2", "4", "6", "8", "10"]),          // evenly spaced, NOT contiguous
        col(&["1", "1", "2", "2", "3", "3"]),           // duplicated, not near-unique
        col(&["-1", "0", "1", "2", "3"]),               // negative present
        col(&["5", "9", "1", "3", "7", "2", "4", "6"]), // shuffled contiguous 1..9
        col(&["1", "2", "3"]),                          // too few
        col(&["a", "b", "c", "d", "e", "f"]),           // non-integer
    ];
    for c in &cases {
        let stat = ColumnScanStats::from_values(c).is_increment();
        let veto = values_form_increment(c) == Some(true);
        assert_eq!(stat, veto, "increment parity mismatch on {:?}", c);
    }
}
