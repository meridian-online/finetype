...
///
/// The taxonomy's `identity.commerce.isbn` validation only checks digit *shape*
/// (`^[0-9]{9}[0-9X]$` or 13 digits), so any 10- or 13-digit number — e.g. a
/// large financial figure like `5150000128` — passes. This applies the real
/// check-digit arithmetic so a genuine ISBN passes and an arbitrary number of
/// the same length does not. Hyphens are allowed (and stripped); a leading sign
/// disqualifies (ISBNs are never signed).
