...
/// Full-column statistics computed once from the materialised column, surfaced
/// to the rule layer / neural short-circuit BEFORE the neural pass (spec
/// 2026-06-16-duckdb-shellout-ingestion ac-04a). FREE: every input
/// (CSV / Parquet / JSON / DuckDB table) already holds the full column at
/// `classify_column`, so this adds no I/O; EXACT (no HLL) — at profile scale a
/// `HashSet` over the column is cheap.
///
/// ac-04a surfaces these; nothing consumes them to change a label yet (parity).
/// ac-04c reads them for the decisive-stat short-circuit, which skips the neural
/// pass when a stat is decisive — defined as ≥98% precise on the columns it fires
/// on, per the ac-04b sweep. The affirmative accessors (`is_increment`,
/// `is_binary_domain`) are the true-branch mirrors of the shipped DEMOTION vetoes
/// (`values_form_increment == Some(true)`, `binary_vocab_veto`'s domain check) —
/// same predicate, asserting rather than refuting. `cardinality_ratio` is the
/// signal the per-value model is structurally blind to (memory
/// `column-stats-separate-residual-gaps`).
#[derive(Debug, Clone, PartialEq)]
pub struct ColumnScanStats {
    /// Count of non-empty (trimmed) values.
    pub non_empty: usize,
    /// Exact distinct count over non-empty string values (cardinality).
    pub distinct: usize,
    /// Count of non-empty values parsing as `i64`.
    pub int_count: usize,
    /// Distinct count among the parsed integers.
    pub int_distinct: usize,
    /// Numeric min over the parsed integers (`None` when none parse).
    pub int_min: Option<i64>,
    /// Numeric max over the parsed integers (`None` when none parse).
    pub int_max: Option<i64>,
}

impl ColumnScanStats {
    /// Compute the full-column stats from the materialised column. Mirrors the
    /// rule layer's null handling — empty (trimmed) values are ignored.
    pub fn from_values(values: &[String]) -> Self {
        ...
        Self {
            ...
            int_count: ints.len(),
            int_distinct,
            int_min: ints.iter().copied().min(),
            int_max: ints.iter().copied().max(),
        }
    }

    /// Cardinality ratio `distinct / non_empty` in `[0, 1]` (0 for an empty
    /// column). Low → a small repeating vocabulary (categorical direction);
    /// ~1 → near-unique (alphanumeric_id / increment direction).
    pub fn cardinality_ratio(&self) -> f32 {
        if self.non_empty == 0 {
            0.0
        } else {
            self.distinct as f32 / self.non_empty as f32
        }
    }

    /// Affirmative increment signature: a CONTIGUOUS, near-unique run of
    /// non-negative integers (`distinct ≈ max−min+1`, almost no duplicates). The
    /// true-branch mirror of `values_form_increment == Some(true)`. Default knob
    /// settings (0.80 contiguity, 0.90 near-unique, ≥5 distinct) match the shipped
    /// veto; the ac-04b sweep tunes them for the skip.
    pub fn is_increment(&self) -> bool {
        if self.int_count < 5 {
            return false;
        }
        let (min, max) = match (self.int_min, self.int_max) {
            (Some(a), Some(b)) => (a, b),
            _ => return false,
        };
        if min < 0 || self.int_distinct < 5 {
            return false;
        }
        let all_int = self.int_count as f32 >= 0.95 * self.non_empty.max(1) as f32;
        let span = (max - min + 1) as f64;
        let contiguous = span > 0.0 && self.int_distinct as f64 / span >= 0.80;
        let near_unique = self.int_distinct as f64 / self.int_count as f64 >= 0.90;
        all_int && contiguous && near_unique
    }

    /// Affirmative exact-binary domain: every non-empty value is an integer in
    /// `{0, 1}`. The true-branch mirror of `binary_vocab_veto`'s domain check.
    pub fn is_binary_domain(&self) -> bool {
        self.non_empty > 0
            && self.int_count == self.non_empty
            && matches!(self.int_min, Some(m) if m >= 0)
            && matches!(self.int_max, Some(m) if m <= 1)
    }
}
