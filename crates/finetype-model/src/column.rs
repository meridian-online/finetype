...
fn value_sharpen(
    ...
    // Rule 26: country_code promotion gate (v23 spec ac-04).
    // Promote a non-geography label to country_code when values are ISO
    // 3166-1 alpha-2 AND the distribution rules out a US-state column.
    //
    // Two-enum check: the alpha-2 pattern alone would falsely promote
    // US-state columns whose state codes happen to also be ISO codes
    // (CA, IL, GA, LA, MS, MD, DE, IN, VA, KY, AL...). The anti-enum
    // partition rejects state-dominated columns.
    //
    // Guards:
    //   - result_label NOT starting with "geography." — preserves any
    //     Sense geography decision (the strong-classes guard).
    //   - n_values ≥ 5 (standard floor).
    //   - ≥80% of non-empty values in ISO 3166-1 alpha-2.
    //   - state_only_rate ≤ 5% (excludes state-coded columns).
    //   - iso_only distinct count ≥ 30% of distinct enum matches
    //     (excludes columns dominated by state_overlap codes).
    if let Some((label, rule)) = sharpen_country_code_promote(values, result_label) {
...
/// R26 — country_code promotion gate.
///
/// Promotes a non-geography label to `geography.location.country_code` when
/// the column's values are ISO 3166-1 alpha-2 codes AND the distribution
/// rules out a US-state column (the dominant precision risk for a naive
/// 2-letter enum check). See spec 2026-05-26-v23-sharpen-code-discriminator
/// ac-04 for the full design and the precision argument.
fn sharpen_country_code_promote(
    values: &[String],
    result_label: &str,
) -> Option<(String, String)> {
    // Strong-classes guard: never override any Sense geography decision.
    if result_label.starts_with("geography.") {
        return None;
    }
    ...
    let iso = crate::country_code_enums::iso_3166_alpha2_set();
    let states = crate::country_code_enums::us_state_alpha2_set();

    // Walk the values once; bucket each into iso-only, state-overlap,
    // state-only, or neither.
    let mut iso_match = 0usize;
    let mut state_only_match = 0usize;
    let mut iso_only_distinct: std::collections::HashSet<&str> =
        std::collections::HashSet::new();
    let mut overlap_distinct: std::collections::HashSet<&str> = std::collections::HashSet::new();

    for v in &non_empty {
        // Only consider 2-char uppercase ASCII tokens.
        if v.len() != 2 || !v.chars().all(|c| c.is_ascii_uppercase()) {
            continue;
        }
        let in_iso = iso.contains(v);
        let in_states = states.contains(v);
        if in_iso {
            iso_match += 1;
            if in_states {
                overlap_distinct.insert(v);
            } else {
                iso_only_distinct.insert(v);
            }
        } else if in_states {
            state_only_match += 1;
        }
    ...
    let iso_rate = iso_match as f32 / non_empty.len() as f32;
    let state_only_rate = state_only_match as f32 / non_empty.len() as f32;
    let total_enum_distinct = iso_only_distinct.len() + overlap_distinct.len();
    let iso_only_distinct_rate = if total_enum_distinct == 0 {
        0.0
    } else {
        iso_only_distinct.len() as f32 / total_enum_distinct as f32
    };

    // All four conditions must hold:
    //   1. ≥80% of values are in ISO 3166 alpha-2
    //   2. ≤5% of values are state-only codes (NY/TX/FL/OK/...)
    //   3. ≥30% of distinct enum-matching codes are ISO-only
    //      (excludes columns where every match is also a US state code)
    //   4. ≥3 distinct enum-matching codes (excludes low-cardinality
    //      columns where the values coincide with one ISO code — e.g.
    //      all "NO" boolean answers being mistaken for Norway, all "AT"
    //      prepositions being mistaken for Austria, single-value-overlap
    //      stock-ticker dumps)
    if iso_rate >= 0.80
        && state_only_rate <= 0.05
        && iso_only_distinct_rate >= 0.30
        && total_enum_distinct >= 3
    {
        Some((
            "geography.location.country_code".to_string(),
            format!(
                "country_code_promote:iso_rate={:.2},state_only={:.2},iso_only_distinct={:.2},n_distinct={}",
                iso_rate, state_only_rate, iso_only_distinct_rate, total_enum_distinct
            ),
        ))
    } else {
        None
...
"#;
    ...
    // ── R26: country_code promotion gate tests (v23 ac-04) ────────────

    #[test]
    fn r26_promotes_international_country_codes() {
        // Sense calls a column categorical; values are clearly international
        // country codes (mostly non-state-overlap). R26 should fire.
        // Pattern observed: corpus columns of country abbreviations
        // misclassified by Sense as `representation.discrete.categorical`.
        let values: Vec<String> = ["US", "GB", "DE", "FR", "JP", "BR", "AU", "MX", "CN", "ZA"]
            .into_iter()
            .map(String::from)
            .collect();
        let result =
            value_sharpen(&values, "representation.discrete.categorical", 0.8, None);
        assert!(result.is_some(), "R26 should fire on international country codes");
        let (label, rule) = result.unwrap();
        assert_eq!(label, "geography.location.country_code");
        assert!(rule.starts_with("country_code_promote"), "got rule: {rule}");
    }

    #[test]
    fn r26_refuses_us_state_only_column() {
        // 10 US state codes — most are NOT in ISO 3166 (NY, TX, FL, OK,
        // OH, MI, PA, NC, WA, WI). state_only_rate ≫ 5%, so R26 must refuse.
        let values: Vec<String> = ["NY", "TX", "FL", "OK", "OH", "MI", "PA", "NC", "WA", "WI"]
            .into_iter()
            .map(String::from)
            .collect();
        let result =
        ...
        assert!(
            result.is_none(),
            "R26 must NOT fire on a US-state column dominated by state-only codes"
        );
    }

    #[test]
    fn r26_refuses_ambiguous_state_overlap_column() {
        // Every value is in BOTH ISO and the US-state set. iso_only_distinct
        // is empty → iso_only_distinct_rate = 0 < 0.30, so R26 must refuse.
        // This protects columns of US states that happen to draw only from
        // the overlap set (CA, IL, GA, LA, MS, MD, DE, IN, VA, KY, AL).
        let values: Vec<String> = ["CA", "IL", "GA", "LA", "MS", "MD", "DE", "IN", "VA", "KY"]
            .into_iter()
            .map(String::from)
            .collect();
        let result =
        ...
        assert!(
            result.is_none(),
            "R26 must NOT fire on all-overlap column (ambiguous between countries and states)"
        );
    }

    #[test]
    fn r26_strong_classes_guard_protects_geography_label() {
        // Even with international country code values, R26 must NOT touch a
        // result already labelled as some geography subtype (region, country,
        // city, ...). Sense's geography decision wins.
        let values: Vec<String> = ["US", "GB", "DE", "FR", "JP", "BR", "AU", "MX"]
            .into_iter()
            .map(String::from)
            .collect();
        for label in [
            ...
            let result = value_sharpen(&values, label, 0.8, None);
            assert!(
                result.is_none(),
                "R26 must respect Sense's geography decision on {label}; got {result:?}"
            );
        }
    }

    #[test]
    fn r26_refuses_below_min_values() {
        // 4 values — under the n=5 floor.
        let values: Vec<String> = ["US", "GB", "DE", "FR"]
            .into_iter()
            .map(String::from)
            .collect();
        let result =
            value_sharpen(&values, "representation.discrete.categorical", 0.8, None);
        assert!(result.is_none(), "R26 must respect the n_values >= 5 floor");
    }

    #[test]
    fn r26_refuses_single_distinct_boolean_collision() {
        // The dominant precision hole found via offline re-apply: a column
        // of all "NO" boolean answers — NO is in ISO (Norway), so without
        // a distinct-count floor R26 would falsely promote 730+ boolean
        // columns corpus-wide. The n_distinct ≥ 3 guard catches this.
        // Target the rule directly (value_sharpen runs other rules too,
        // and downstream rules may fire on these contrived inputs).
        let values: Vec<String> = vec!["NO".to_string(); 10];
        let result = sharpen_country_code_promote(&values, "representation.boolean.terms");
        assert!(
            result.is_none(),
            "R26 must NOT fire on single-distinct boolean column (all NO); got {result:?}"
        );

        // Same precision class — all "IT" pronouns, all "AT" prepositions,
        // all "AS" conjunctions, all "BY" prepositions, all "IN" prepositions.
        for tok in ["IT", "AT", "AS", "BY", "IN", "ME"] {
            let values: Vec<String> = vec![tok.to_string(); 8];
            let result = sharpen_country_code_promote(
                &values,
                "representation.discrete.categorical",
            );
            assert!(
                result.is_none(),
                "R26 must NOT fire on single-distinct collision: all '{tok}'; got {result:?}"
            );
        }
    }

    #[test]
    fn r26_fires_at_iso_rate_boundary() {
        // 8 ISO codes + 2 garbage tokens — iso_rate = 80%, at the boundary,
        // and the iso_only distinct set dominates. Should fire.
        let values: Vec<String> = ["US", "GB", "DE", "FR", "JP", "BR", "AU", "MX", "ZZ1", "abc"]
            .into_iter()
            .map(String::from)
            .collect();
        let result =
            value_sharpen(&values, "representation.discrete.categorical", 0.8, None);
        assert!(result.is_some(), "R26 should fire at the 80% iso_rate boundary");
    }
}
