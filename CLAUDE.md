...
- **Validate-precision corpus shipped — iter 1 (spec `orbit/specs/2026-04-28-validate-precision-corpus/`).** `make validate-corpus` round-trip harness over 7 real-world CSVs (pokemon, rio2016_athletes, us_baby_names, co2_emissions_by_nation, world_population, un_locode, global_temp_annual); baseline `3 of 7 datasets pass at P=99%`. In-scope fixes shipped: ac-09 (`finetype profile --enum-threshold` default 50→32 + `representation.boolean.*` gate parity for JSON Schema enum emission) and ac-10 (`representation.numeric.decimal_number` validator widened to accept scientific notation). Misclassification + code_vs_canonical mechanisms deferred to follow-up cards (await retrain / cross-cutting taxonomy decision). Three MADRs land: 0072 (round-trip pass as the validate metric), 0073 (m-19 infrastructure reuse), 0074 (in-scope vs deferred fix mechanisms). Iter-2 ships 5 mechanism-coverage datasets (NYC Taxi, GDELT, FIFA, S&P 500, OECD employment) under spec `orbit/specs/2026-04-28-validate-corpus-curation/`; harness mechanism-attribution gap surfaced and tracked as a stub spec at `orbit/specs/2026-04-28-validate-corpus-iter3/`. Both specs hang under user-feature card 0014 (`orbit/cards/0014-profile-validate-precision.yaml`).
