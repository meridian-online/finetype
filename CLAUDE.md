...
- **Validate-corpus shipped through iter-3 — mechanism attribution refactor (spec `orbit/specs/2026-04-28-validate-corpus-iter3/`).** Iter-1 (`orbit/specs/2026-04-28-validate-precision-corpus/`) shipped the `make validate-corpus` round-trip harness over 7 real-world CSVs (3 of 7 datasets pass at P=99%; ac-09 enum threshold 50→32 + boolean gate parity, ac-10 scientific notation in `representation.numeric.decimal_number`). Iter-2 (`orbit/specs/2026-04-28-validate-corpus-curation/`) added 5 mechanism-coverage datasets (NYC Taxi, GDELT, FIFA, S&P 500, OECD employment); 12 datasets / 46622 rows total, 3 of 12 pass. Iter-3 rewrites the harness's mechanism-attribution cascade as 7 explicit rules emitting `(Mechanism, &'static str)` tuples (6 trigger labels: `path-a-pattern`, `path-b-prefix`, `path-b-codetype`, `enum-constraint`, `prediction-error`, `fallthrough`); per-column table gains a `Trigger` column; analyst-facing doc at `docs/mechanism-attribution.md`; 80-row fixture (`eval/datasets/validate_corpus_expected_attributions.yaml`) as anti-regression lock with `pending_escalation` flag for known-pending rows (REF_AREA model collision, GICS Sector taxonomy gap). 21 `vci3_*` tests. Six MADRs land across the three iterations: 0072 (round-trip pass as the validate metric), 0073 (m-19 infrastructure reuse), 0074 (in-scope vs deferred fix mechanisms), 0075 (mechanism bucket coalesce), 0076 (fixture anti-regression lock), 0077 (label-only code-vs-canonical attribution). All three specs hang under user-feature card 0014 (`orbit/cards/0014-profile-validate-precision.yaml`).
