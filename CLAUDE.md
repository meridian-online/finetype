...
**Shipped default Sense-stage model:** `models/default` → **`sherlock-v19-relu-s42`** — **BEING RETIRED** (240-label + unreproducible; see the "v19 is retired" architectural-direction entry). Still the shipped binary that every `finetype` invocation runs (unless `FINETYPE_MODEL` overrides) ONLY until the reproducible 244-label successor clears the blocking corpus-honest gate. v19 baseline: profile eval 369/448 on the 448-row manifest; gold composed 0.793 / Sense 0.502. **Do not gate new work against v19 — gate against the reproducible baseline.**
