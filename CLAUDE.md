...
**The headline accuracy eval is the gold corpus**: `eval/gold/gold_corpus_v1.tsv` (931 human-or-calibrated-verified columns, per-row provenance, leakage-firewalled) scored by `scripts/score_gold_anchor.py` (`build-gold` → `predict` → `score`; per-type precision/recall with Wilson CIs + one headline number). v19 baseline = 0.655 (CI 0.624–0.685). Gold labels are append-only facts; expansion is emission-driven each promotion round; the fixture records its taxonomy version (full evolution policy in choice 0095).

**Gold v2 (2026-06-16, spec `2026-06-16-mixed-teacher-gold-readjudication`):** `eval/gold/gold_corpus_v2.tsv` applies 24 re-adjudication label changes to the `ac-03`-heuristic tier (23 mixed-panel Opus/Sonnet/Haiku blind + 1 author override `idPedido` integer→unix_milliseconds), plus 23 author-confirmed provenance upgrades — author-accepted, append-only (old label preserved in provenance), leakage firewall re-verified (0 overlap, identical column membership). Shipped model: v1 738/931 = 0.793 → **v2 742/931 = 0.797** (+4 — the tiny shift is the integrity proof that re-adjudication cleans gold rather than inflating the score; of the panel corrections 37/47 disagreed with the model, 8 helped / 5 hurt / 10 neutral). At the author gate the 24 deferred proposals resolved to **23 CONFIRM gold + 1 override** — the panel over-reached on numeric→alphanumeric_id and TLD→categorical, author kept them numeric/TLD. v2 is the corrected canonical fixture going forward; v1 is historical. FOLLOW-UPS open: phase-2 header-heuristic tier (240 cols), a datetime constrained-vocabulary re-run (CONTESTED inflated by iso/calendar_date/date vocab noise), and wiring v2 as the default `score_gold_anchor.py` path.

**Promotion order (0095):** gold-anchor (efficacy) → drift proxy (pre-train) → **gold corpus accuracy + rare-type scoreboard** (headline; scoreboard validated at 99.0%) → corpus-honest gate (**blocking**, H05 — the only relocation detector, role unchanged) → swap. No headline ever overrides a blocking NO-GO.
