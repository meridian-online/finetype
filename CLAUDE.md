...
- **Next accuracy targets** — 7 misses at 113/120: countries.name (entity_name overcall — correct demotion but GT is geography), world_cities.name (last_name overcall — geography protection doesn't guard last_name from Model2Vec), books_catalog.publisher (city overcall, GT expects full_name), tech_systems.server_hostname (hostname prediction, GT expects full_name), people_directory.company (categorical, GT expects full_name), codes_and_ids.cvv (postal_code overcall), codes_and_ids.swift_code (sedol overcall). **Model state:** v0.3.0 models (169 types) restored from HuggingFace; taxonomy has 171 types (entity_name + paragraph added but not in model). Entity classifier is a separate model (4-class MLP, not part of CharCNN). CLDR date/time patterns and 4-level locale labels (NNFT-126) are next infrastructure pieces.
