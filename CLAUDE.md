...
**Gold v2 (2026-06-16, spec `2026-06-16-mixed-teacher-gold-readjudication`):** `eval/gold/gold_corpus_v2.tsv` applies 23 mixed-panel (Opus/Sonnet/Haiku, blind) re-adjudication corrections to the `ac-03`-heuristic tier — author-accepted, append-only (old label preserved in provenance), leakage firewall re-verified (0 overlap). Shipped model: v1 738/931 = 0.793 → **v2 741/931 = 0.796** (+3: 8 corrections helped the model, 5 hurt it, 10 neutral — the tiny shift is the integrity proof that re-adjudication was not goalpost-moving; 37/47 proposed corrections disagreed with the model). v2 is the corrected canonical fixture going forward; v1 is historical. FOLLOW-UPS open: phase-2 header-heuristic tier (240 cols), the 24 deferred proposals (18 numeric→alnum, 5 TLD→categorical, 1 low-conf — many defensible, held back conservatively; TLD→categorical is the real judgment call), a datetime constrained-vocabulary re-run, and wiring v2 as the default `score_gold_anchor.py` path.
