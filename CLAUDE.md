...
- **v19 retrain — ELIGIBLE but not forced.** The amount-variant cluster shipped as a pipeline-layer fix (decision 0065; no retrain needed). Remaining v18 follow-up clusters — container-type collapse (8 columns → `categorical`) and datetime-subtype collapse (6 → nearest-but-wrong timestamp) — are candidates for the same diagnostic arc per MADR 0067 (retrain is not the default hypothesis for header-driven subtype collapse). Run `/orb:discovery` on each cluster before committing to training-layer work. Any v19 retrain that does ship must pass the hard gate in MADR 0066 (3-seed sweep + net_label_delta ≥+1 + net_domain_delta ≥0 + per-domain regression ≤3 + per-column diff + mechanism attribution). v4-UA-adoption card (decision 0060 follow-up) also carries forward.
