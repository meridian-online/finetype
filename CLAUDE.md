...
- **v18 retrain — UNBLOCKED by m-19 Phase A+B.** Expanded 352-col eval is live, leakage firewall is on, coverage floor at 240/240. v16 diagnostic baseline on expanded corpus: 297/352 (84.4% label, 91.8% domain). Open design question: does v18 rebuild training data from v16's corpus (skip v17), or inherit v17's v4 loaders + generator improvements despite v17's net-zero eval delta? Discovery first — triage the ~55 v16 failures on the expanded eval to decide per-type whether data / Sharpen rule / architecture is the lever. Sprint-next.
