...
- **Entity classifier integrated** (NNFT-151, NNFT-152) — Deep Sets MLP entity classifier now runs in Rust column inference pipeline. When CharCNN votes full_name, the entity classifier checks if column values are non-person entities (organizations, places, creative works) and demotes to entity_name. Architecture: 300-dim features (128 emb_mean + 128 emb_std + 44 statistical features) → BatchNorm → 3×Linear/ReLU → 4-class softmax. Shares Model2Vec tokenizer+embeddings with SemanticHintClassifier. Entity demotion guard prevents header hints from overriding entity demotion decisions. **SOTAB**: 42.5% → 43.3% label (+0.8pp), 64.4% → 68.3% domain (+3.9pp). Entity demotion fires on 3,027 SOTAB columns (18.1%). **Profile eval**: 113/120 (94.2%) unchanged — no regressions. Model artifacts in `models/entity-classifier/`, Rust code in `crates/finetype-model/src/entity.rs`.
- **Next accuracy targets** — 6 misses at 114/120 (NNFT-154 triage, NNFT-155 GT fixes): codes_and_ids.cvv (postal_code overcall — numeric disambiguation prevents header hint override), world_cities.name (last_name overcall — geography protection doesn't guard last_name from Model2Vec), codes_and_ids.swift_code (sedol overcall — SEDOL validation should fail on 8-char SWIFT codes), countries.name (entity_name overcall — correct demotion but GT expects geography.location.country), people_directory.company (categorical vs entity_name — entity demotion doesn't fire because top vote isn't full_name), books_catalog.publisher (city vs entity_name — low-confidence model confusion). **Rule-fixable:** cvv (hint priority for numeric disambiguation), world_cities.name (extend geography protection to last_name), countries.name (geography rescue during entity demotion). **Model state:** v0.3.0 models (169 types) restored from HuggingFace; taxonomy has 171 types (entity_name + paragraph added but not in model). Entity classifier is a separate model (4-class MLP, not part of CharCNN). CLDR date/time patterns and 4-level locale labels (NNFT-126) are next infrastructure pieces.
...
**Core benchmarks:**
1. **Profile eval** (`eval/profile_eval.sh`) — Regression smoke test on annotated CSV datasets. Scores against `schema_mapping.yaml`. Current: 95.0% label accuracy (114/120), 96.7% domain accuracy (116/120) on format-detectable (direct+close) types across 21 datasets.
