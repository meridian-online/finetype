...
- **v0.6.17 release** — Promoted sherlock-v16 as `models/default`, published to HuggingFace, bumped version 0.6.16 → 0.6.17. Release card: `specs/2026-04-20-v16-release/`.
...
- **CI hygiene — decouple from `models/default`** — `.github/scripts/download-model.sh` reads the live symlink at CI time. This forces release + model publish to interleave awkwardly (as seen in the v0.6.17 release). Pin the CI model name via env var or a separate file so promotion PRs don't deadlock. Small card, ~1–2 hours.
