...
**CI status:** All checks pass (fmt, clippy, test, taxonomy check). 406 model tests, zero warnings.
...
- **Validate-corpus iter-4 — `finance.currency.amount` bare-decimal widening (SHIPPED 2026-04-29)** — `finance.currency.amount`'s `validation.pattern` at `labels/definitions_finance.yaml:131` gains a 4th alternation borrowed verbatim from `representation.numeric.decimal_number`'s pattern at `definitions_representation.yaml:79`: `^-?[0-9]+(\.[0-9]+)?([eE][+-]?[0-9]+)?$`. Closes the format-diversity gap surfaced by the discovery vehicle `eval/datasets/csv/ecommerce_orders.csv` — `total_price` reject count drops from **63 → 0** (4-digit unsymboled values like `1914.96` previously failed the comma-required alternations 1–3). YAML comment block above the pattern names alternations 1–4, source path, and MADR 0078 for traceability. iter-3's `vci3_fixture_attribution_regression_match` continues to pass; zero substantive row-level drift on the 12-dataset corpus (regenerated `validate_corpus.md` differs only in the `Generated:` timestamp). Three new `vci4_*` regression tests in `crates/finetype-eval/src/bin/validate_corpus.rs` (bare-decimal acceptance, format-preservation, non-money rejection) — all pass. Two misclassification findings deferred to follow-up card 0015 with diagnostic detail (`status` → `datetime.component.periodicity`, `order_id` → `finance.securities.sedol`). Decision: **0078** (validator alternations may compose canonical sibling-type patterns — composition over invention). Spec: `orbit/specs/2026-04-29-validate-corpus-iter4/`.
- **Validate absorbs load — v0.6.19 (IN FLIGHT 2026-04-28, rally branch)** — Hard fold of `finetype load` into `finetype validate --db --table`. `Commands::Load`, `cmd_load`, `build_load_expr`, `build_load_expr_enum` and the orphan `sanitise_identifier` deleted outright (~270 LOC + 6 obsolete unit tests). Per-column transform logic lifted to `build_transform_projection(headers, extensions, taxonomy, try_wrap)` adjacent to `cmd_validate_table`. Materialise path now runs a pre-CTAS sweep: per typed column, `INSERT INTO __finetype_transform_failures_<uuid> SELECT row_idx, ... WHERE col IS NOT NULL AND TRY(transform) IS NULL`, then user-table CTAS excludes those `__row_idx`s via `NOT IN`. Reject ontology extended: `error_type` gains `'TRANSFORM_FAILED'`, `constraint_failed` gains `'transform'` — REJECT_SIDECAR_DDL 13 cols unchanged. `error_message` semantic split: SEMANTIC_TYPE rows carry engine failure reason; TRANSFORM_FAILED rows carry literal `"transform_failed: <transform-expr>"`. ENUM emission dropped; `--enum-threshold` not carried over. NULL-in-NULL-out preserved (staging-NULL → typed-NULL is not a transform failure). MCP `ValidateRequest` byte-identical; description string at `lib.rs:113` and `tools/validate.rs` doc comment carry the addendum. `finetype load …` errors via clap unknown-subcommand handler (exit 2) — no shim. Public CLI surface drops to **5 verbs**: infer, profile, validate, mcp, taxonomy. 13 ACs, 13 vrp_* CLI tests (12 prior + ac-06 verification), 5 unit tests for `build_transform_projection`. Decisions: 0071 (this fold; refines 0064). Spec: `orbit/specs/2026-04-28-validate-absorbs-load/`.
- **CLI visibility cleanup — v0.6.19 (SHIPPED 2026-04-28, PR #51)** — Reduces `finetype --help` to 7 public commands (infer, profile, schema, validate, load, mcp, taxonomy); 6 internal commands hidden via `#[command(hide = true)]` (check, generate, train, train-multi-branch, eval, infer-batch). Removed entirely: `--model` flag (replaced by `FINETYPE_MODEL` env var; 9 script loci migrated), `--sharp-only` flag (with all dead-gate code collapsed), `eval-gittables` subcommand (~568 LOC; standalone binary at `finetype-eval/src/bin/eval_gittables_cli.rs` untouched). Schema export reduced to `x-finetype-label` + `x-finetype-pii` only — 6 derivable extensions dropped (broad-type, transform, transform-ext, format-string, domain, confidence) from both type-mode and table-mode emitters. `finetype validate` defaults to check-only mode; `--db`/`--table` are optional-but-mutually-required via clap `requires`; `--append` requires `--db`. CLAUDE.md gains "Public vs internal CLI surface" section. 10/10 ACs, 11/11 constraints, +336/-887 LOC. Spec: `orbit/specs/2026-04-27-cli-visibility-cleanup/`.
- **Sharpen rule audit + v19-relu model promotion (IN PROGRESS 2026-04-27)** — Full ablation of all Sharpen post-processing rules against v19-relu-s42 (best ReLU seed from overnight 3-seed sweep, val_acc 0.9173). Ablation script (`scripts/sharpen_ablation.sh`) profiles all 448 manifest rows with and without Sharpen, measures per-rule net impact. Removed 3 net-negative/net-zero rules: `disambiguate_small_integer_ordinal` (net -2), `categorical_low_cardinality` (net -1), `categorical_single_char` (net 0). Demotion guard (MADR 0059) removed alongside — no demotion rules remain to guard. Post-cleanup score: 369/448 (+4 from rule removal, +4 over pre-cleanup 365). Gate: MADR 0069 amends 0066 to accept candidate-pre-cleanup parity when rules decrease. Per-column diff: 14 fixes / 16 regressions / 355 stable_hit. v19-relu-s42 promoted as `models/default`. Decisions: 0068 (GELU+LN outcome), 0069 (gate amendment). Spec: `orbit/specs/2026-04-27-sharpen-rule-audit/`.
- **Amount-variant collapse — pipeline-layer fix (SHIPPED 2026-04-24, PR #47)** — 11 `finance.currency.amount_*` subtypes collapsed to plain `finance.currency.amount` in v16 and v18 (47/55 v16 failures carried through v18 retrain unchanged — decision 0062). Diagnostic arc (ac-01..ac-04) ruled out imbalance (corpus max/min=1.214), overlap (mean Jaccard=0.0102), confident_wrong (only 4/11), and flat_confidence (only 1/11). Primary mechanism named `other` in MADR 0065: `header_hint()` at `crates/finetype-model/src/column.rs:4303-4314` over-generalised on the `amount` substring and forced every variant header back to plain amount via Sharpen hint-override branches. Fix: 11-arm addition to the `match h` block (short-circuits the destructive substring matcher). **No retrain**, **no training data changed**. Target lift: **+11** (ac-09 GO, threshold +3). Non-target regression delta: **+10** (ac-10 PASS, threshold -1). Decisions 0065 (mechanism), 0066 (v19 retrain hard gate), 0067 (framing correction — retrain is not the lever for header-hint failures). Spec: `orbit/specs/2026-04-24-amount-variant-generators/`.
...
- **Validate-corpus arc shipped through iter-4 — `finance.currency.amount` bare-decimal widening (spec `orbit/specs/2026-04-29-validate-corpus-iter4/`).** Iter-1 (`orbit/specs/2026-04-28-validate-precision-corpus/`) shipped the `make validate-corpus` round-trip harness over 7 real-world CSVs (3 of 7 pass; enum threshold 50→32 + boolean gate parity; scientific notation absorbed into `representation.numeric.decimal_number`). Iter-2 (`orbit/specs/2026-04-28-validate-corpus-curation/`) added 5 mechanism-coverage datasets (NYC Taxi, GDELT, FIFA, S&P 500, OECD employment); 12 datasets / 46622 rows. Iter-3 (`orbit/specs/2026-04-28-validate-corpus-iter3/`) rewrote the harness's mechanism-attribution cascade as 7 explicit rules emitting `(Mechanism, &'static str)` tuples (6 trigger labels) with an 80-row fixture anti-regression lock and analyst-facing doc at `docs/mechanism-attribution.md`. Iter-4 widens `finance.currency.amount`'s `validation.pattern` at `labels/definitions_finance.yaml:131` to absorb bare-decimal money values (`1914.96`, `19.95`, `100`, `-50.5`, scientific notation) by composing — verbatim — `representation.numeric.decimal_number`'s canonical pattern as the 4th alternation. Discovery vehicle (synthetic `eval/datasets/csv/ecommerce_orders.csv`) sees `total_price` reject count drop **63 → 0**; `status` (→`datetime.component.periodicity`) and `order_id` (→`finance.securities.sedol`) misclassifications recorded as deferred follow-up card 0015. iter-3's `vci3_fixture_attribution_regression_match` continues to pass (zero row-level drift on the 12-dataset corpus). Three new `vci4_*` regression tests. Seven MADRs land across the four iterations: 0072 (round-trip pass as the validate metric), 0073 (m-19 infrastructure reuse), 0074 (in-scope vs deferred fix mechanisms), 0075 (mechanism bucket coalesce), 0076 (fixture anti-regression lock), 0077 (label-only code-vs-canonical attribution), 0078 (composition over invention — validator alternations may compose canonical sibling-type patterns). All four specs hang under user-feature card 0014 (`orbit/cards/0014-profile-validate-precision.yaml`).
- **v19-relu promoted; remaining regressions are model-intrinsic.** 16 regressions vs v16: 7 on coverage_closure (synthetic), 5 cross-domain/scientific (inchi, smiles, iban, swift_code, order_id), 4 datetime subtypes. 7 header hint matches remain net-negative (net -1 each) — deferred to model improvement rather than hint removal. Next diagnostic arc: investigate the 5 cross-domain regressions to determine if they're addressable by training data or if they represent genuine model limitations.
- **N=1 email regression in v16** — single-value email in `--mode column` returns `plain_text`; v14 returned `email`. At N≥5 classifies correctly. May behave differently under v19. Interview: `orbit/specs/2026-04-20-v16-n1-email-regression/interview.md`.
- **2 actionability below-target** — multilingual/date (33.3%), tech_systems/version (93.8%). Both misclassification-driven; may resolve under v19.
...
As of v0.6.19, `finetype --help` lists **only the 5 public commands**.
...
change shape between minor versions without a deprecation cycle.
...
The hide mechanism is `#[command(hide = true)]` on the clap variant —
no wrapper scripts, no env-var gating, no separate binary. Hidden ≠
removed: `finetype check` continues to power `make ci` and
`finetype generate` continues to power training data prep.

`--model` was removed from every subcommand in v0.6.19. The model
directory is now configured exclusively via the `FINETYPE_MODEL` env
var (default: `models/default`). See the **Model-name env vars**
section below.
...
48 architectural decisions in `orbit/decisions/` (MADR format). Key decisions — do not revisit without good reason.
